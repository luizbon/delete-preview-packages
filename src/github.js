...
async function getAllPackages(params) {
    ...
    const promises = params.packages.map(async (packageName) => {
        try {
            ...
            const currentVersion = versions.find(({ name }) => semver.prerelease(name) === null);

            return versions.filter(({ name }) => semver.prerelease(name) && semver.lt(name, currentVersion.name));
        } catch (error) {
            return [];
        }
    });
